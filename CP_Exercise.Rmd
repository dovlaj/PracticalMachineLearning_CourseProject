...
The data set has 160 variables, but by careful removal of mostly-NA variables and keeping only the most significant ones, it can be diminished to 20 variables. With these 20 variables, we can get ~99% accuracy when using random forests. It is likely that this is due to overfitting because of small number of individuals and we cannot guarantee that we will get similar results in general case (when applie to larger public).
